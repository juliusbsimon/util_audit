...
BEGIN
    ...
    FOR i IN 1 .. v_cols.COUNT LOOP
        ...
        '  IF NOT INSERTING THEN v_old_row.put('''||v_cols(i)||''', :OLD.'||v_cols(i)||'); END IF;'||CHR(10)||
        '  IF NOT DELETING THEN v_new_row.put('''||v_cols(i)||''', :NEW.'||v_cols(i)||'); END IF;'||CHR(10)||
        '  IF NVL(:OLD.'||v_cols(i)||',''§'') <> NVL(:NEW.'||v_cols(i)||',''§'') THEN'||CHR(10)||
